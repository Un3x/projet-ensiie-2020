<?php
	...
	if (!empty($_POST)){
	  	...
	  	if (isset($_POST['ajout-oeuvre'])){
		    ...
		    if($valid){
		        $ajout = $dbAdapter->prepare('INSERT INTO Oeuvre(titre,lien_photo,synopsis,proposition) VALUES(?, ?, ?, ?)');
		        $ajout->execute(array($oeuvre,$lien_photo,$synopsis,false)); 
		        header('Location: ajouter-oeuvre.php');
		        exit;
	      }
	  }
	}
...
<body>
	...
	<form method="post">
		<div class="form-group">
			<div>Nom de l'oeuvre : <input type="text" name="oeuvre"></div>
			<div>Synopsis de l'oeuvre : <textarea class="form-control" name="synopsis" rows="4" placeholder="..."></textarea></div>
			<div>Image associée à l'oeuvre : <input type="file" name="photo" accept="image/*"></div>
		</div>
		<div class="form-group">
