...
public function getReunion($meeting)
{
    ...
    public function newReunion($Nom_Assoc, $IdReu, $Date_debut_reu, $Date_fin_reu, $Id_MembreA, $Descriptif)
    {
        ...
        $req=$this->dbAdapter->prepare("INSERT INTO Reunion(Id_Assoc,Id_reu, Date_debut_reu,Date_fin_reu, Id_MembreA, Descriptif) 
                                        VALUES(:id_assoc,:id_reu, :date_debut_reu, :date_fin_reu, :id_membrea, :descriptif)");
...
/**
    ...
    public function reuAsso($id){
        ...
        foreach ($usersAsso as $TteAsso) {
            $userA = new Reunion();
            $userA->setDateDebutReu($TteAsso['date_debut_reu']);
            $userA->setIdReu($TteAsso['id_reu']);
            $Asso[] = $userA;
           }
        return $Asso;
    }
